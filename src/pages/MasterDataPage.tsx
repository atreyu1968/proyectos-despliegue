...
import { generateTemplate, importData, listEntities } from '../services/masterDataService';
import { useAuth } from '../hooks/useAuth';

const MasterDataPage: React.FC = () => {
  const { user } = useAuth();
  const { canEdit } = usePermissions();
  const [selectedType, setSelectedType] = useState<MasterDataType>('centers');
  ...
  const loadData = async () => {
    try {
      const entities = await listEntities(selectedType);
      setData(entities);
    } catch (error) {
    ...
    <div className="p-8">
      ...
      {showImport && (
        <ImportModal
          ...
          onImport={async (file) => {
            if (!user) return { success: false, totalRows: 0, successCount: 0, errorCount: 1, errors: [] };
            const result = await importData(selectedType, file, user.id, user.name);
            if (result.success) {
              loadData();
            }
            return result;
